...
date: 2013-10-19 20:11:00 -0400
categories: [iOS, unit-testing]
---
...
The only way to stub a class method is via swizzling, which can get messy. The alternative is dependency injection, whereby you expose object dependencies using properties or constructor variables. Then they can be swapped more easily at runtime, so if the `AFRedditAPIClient` object were a public property, and `RCMessageClient` were the class you wanted to test, expose the dependency using a property and in the test assign it to the mock object:
